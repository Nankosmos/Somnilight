...
// ==================== Backend requests ====================
  ...
  const applySleepReport = (data) => {
    ...
    setSleepSource(data.source || null); // 记录来源：mqtt_simulated / http_simulated
  ...
  const fetchSleepReport = async (jsDate) => {
    try {
      ...
      // 2️⃣ 如果没有 MQTT，则退回 HTTP 15days_default（模拟）
      if (!report) {
        try {
